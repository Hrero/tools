...
program
  ...
  // 当 .command() 带有描述参数时，不能采用 .action(callback) 来处理子命令，否则会出错。这告诉 commander，你将采用单独的可执行文件作为子命令，就像 git(1) 和其他流行的工具一样。 Commander 将会尝试在入口脚本（例如 ./lib/lzg）的目录中搜索 program-command 形式的可执行文件，例如 lzg-init, lzg-config
  // 指定 opts.isDefault 为 true 将会在没有其它子命令指定的情况下，执行该子命令。例如 lzg  等价于 lzg init
  .command('init <projectName>', '创建新项目', { isDefault: true }).alias('start')
  .command('config [service]', 'stop named service, or all')
