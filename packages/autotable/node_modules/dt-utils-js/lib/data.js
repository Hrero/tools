...
function getTwoArrayObiectKeys(arr, brr, option) {
    ...
    return [].concat(_toConsumableArray(brr));
}
/**
 * 寻找字符串出现最多的字符，并且统计次数
 * @param str
...
function getSortValue(option) {
    ...
    var backArr = [];
    var values = dbj.map(function (ma) {
        return ma[key];
    }).sort(function (a, b) {
        return b - a;
    });
    var newvalues = values.slice(0, max);
    return dbj.filter(function (fl) {
...
exports.default = {
    ...
    uniqueData: uniqueData, getArrIsOne: getArrIsOne, getTwoArrayObiectKeys: getTwoArrayObiectKeys, findMaxDuplicate: findMaxDuplicate, getLongField: getLongField, getSortValue: getSortValue
};
